...
def getcalibrated_b(cropped):
        ...
        while(i < len(cropped)):
                for x in cropped[i]:
                        ...
                        val_b.append(x[2])
                i+=1
        hsv_mode_b.append(findmode(hue_b))
        hsv_mode_b.append(findmode(sal_b))
        hsv_mode_b.append(findmode(val_b))
        print hsv_mode_b
        return hsv_mode_b
        

def getcalibrated_r(cropped):
        i = 0
        while(i < len(cropped)):
                for x in cropped[i]:
                        hue_r.append(x[0])
                        ...
                        val_r.append(x[2])
                i+=1
        hsv_mode_r.append(findmode(hue_r))
        hsv_mode_r.append(findmode(sal_r))
        hsv_mode_r.append(findmode(val_r))
        print hsv_mode_r
        return hsv_mode_r


def getcalibrated_y(cropped):
        i = 0
        while(i < len(cropped)):
                for x in cropped[i]:
                        hue_y.append(x[0])
                        ...
                        val_y.append(x[2])
                i+=1
        hsv_mode_y.append(findmode(hue_y))
        hsv_mode_y.append(findmode(sal_y))
        hsv_mode_y.append(findmode(val_y))
        print hsv_mode_y
        return hsv_mode_y
...
def drawCentroid(vid, color_area, mask, showCentroid):
	...
	im2, contour, _ = cv2.findContours( mask, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
...
def find_range(hue, sat, val, hsv):
...
def calibrateColor(color, def_range):
	...
	#cv2.createTrackbar('Val', name, def_range[0][2]   , 255, nothing)
                while(1):
                        ...
                        if color == 'Yellow':
                                if(k == ord('q')):
                                        hsv_mode_1 = getcalibrated_y(cropped)
                                        hue = hsv_mode_1[0] #cv2.getTrackbarPos('Hue', name)
                                        sat = hsv_mode_1[1] #cv2.getTrackbarPos('Sat', name)
                                        val = hsv_mode_1[2] #cv2.getTrackbarPos('Val', name)
                                        cv2.destroyWindow(name)
                                        return(find_range(hue, sat, val, hsv))
                                elif k == ord('d'):
                                        cv2.destroyWindow(name)
                                        return(def_range)
                        elif color == 'Red':
